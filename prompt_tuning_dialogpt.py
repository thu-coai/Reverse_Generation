...
class Helper:
    ...
    def __init__(self, tokenizer, only_response=False):
        self.tokenizer = tokenizer
    ...
    def collate_fn(self, batch):
        if self.only_response:
            responses = [self.tokenizer.eos_token + sample['response'] + self.tokenizer.eos_token for sample in batch]

            response_ids = [torch.tensor(self.tokenizer.encode(response, add_special_tokens=False), dtype=torch.long) for response in responses]
            pad_token_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else self.tokenizer.eos_token_id
            pad_ids = pad_sequence(response_ids, batch_first=True, padding_value=pad_token_id)
            labels = torch.full_like(pad_ids, -100)
            for i in range(len(response_ids)):
                end = len(response_ids[i])
                labels[i, :end] = pad_ids[i, :end]
...
class PromptTuningModel(nn.Module):
    def __init__(self, args):
        ...
        if args.use_prefix:
            self.prompts = torch.nn.Parameter(torch.randn(self.kind_num, self.num_token, self.n_embd))
    ...
    def init_prompt(self):
        ...
        with torch.no_grad():
            # torch.nn.init.uniform_(self.prompts, -0.5, 0.5)
            for i in range(self.kind_num):
                kind = Helper.all_kinds[i]
                # hard_prompt = f'Given the response, please generate a context that is about {kind}'
                hard_prompt = kind
                ids = self.tokenizer.encode(hard_prompt, add_special_tokens=False)
                logger.debug(f'{kind}, {len(ids)}')
                kind_emb = emb_layer(torch.tensor(ids, dtype=torch.long))
                self.prompts[i, :len(ids)] = kind_emb.detach().clone()
                if self.prompts.size(1) > len(ids):
                    concat_ids = list(range(self.prompts.size(1) - len(ids)))
                    concat_emb = emb_layer(torch.tensor(concat_ids, dtype=torch.long))
                    self.prompts[i, len(ids):] = concat_emb.detach().clone()
                # cnt = self.num_token // len(kind_emb)
                # kind_emb = kind_emb.repeat(cnt + 1, 1)
                # self.prompts[i] = kind_emb[:self.num_token].detach().clone()
    ...
    def forward(self, input_ids, labels, kinds):
        # kinds: int, output type
        if self.args.use_prefix:
            emb_layer = self.model.get_input_embeddings()
            inputs_embeds = emb_layer(input_ids)

            kinds_embs = self.prompts[kinds]
            inputs_embeds = torch.cat([kinds_embs, inputs_embeds], dim=1)
            label_siz = kinds_embs.size()[:-1]
            concat_labels = torch.full(label_siz, -100, device=input_ids.device, dtype=torch.long)
            labels = torch.cat([concat_labels, labels], dim=1)
...
class Arguments:
    ...
    root_dir: Optional[str] = field(default='../logs/dialogpt_prompttune')
    reverse: Optional[bool] = field(default=True)
    onlyresponse: Optional[bool] = field(default=True)
    load_ckpt: Optional[bool] = field(default=False)
    num_token: Optional[int] = field(default=10)
    use_prefix: Optional[bool] = field(default=False)
    fixlm: Optional[bool] = field(default=False)
    prompt_initway: Optional[str] = field(default='kind_rand')
    ...
    def __post_init__(self):        
        ...
        self.save_path_suffix = f'_usePrompt{str(self.use_prefix)}{self.num_token if self.use_prefix else ""}_fixlm{str(self.fixlm)}_promptinit{self.prompt_initway}_lr{self.lr}_onlyresponse{self.onlyresponse}{self.suffix}'
...
def data_process(args):
    ...
    print(f'train length:{len(df_train)}, val length:{len(df_val)}, test length:{len(df_test)}')
...
def main(args):
    ...
    if args.mode == 'data':
        ...
        if args.use_prefix:
            if args.fixlm:
                ...
                for k in current_model.keys():
                    if k in ckpt and current_model[k].size() == ckpt[k].size():
                        new_state_dict[k] = ckpt[k]
                    else:
                        new_state_dict[k] = current_model[k]
                        print(f'not load {k} from ckpt')
                # new_state_dict = {k:ckpt[k] if k in ckpt and current_model[k].size() == ckpt[k].size() else current_model[k] for k in current_model.keys()}
                model.load_state_dict(new_state_dict)

                if args.prompt_initway == 'rand':
                    pass
                elif args.prompt_initway == 'kind_rand':
                    model.init_prompt()
                model.fix_lm()
            ...
            print(f'trainable parameter num:{parameter_num / 1e6} M')
        else:
            model.train()
            model.to(device)
        ...
        helper = Helper(tokenizer, args.onlyresponse)
        ...
        wandb.init(project='dialogpt_prefix_tuning', name=f'usePrompt{args.use_prefix}{args.num_token if args.use_prefix else ""}_fixlm{str(args.fixlm)}_promptinit{args.prompt_initway}_lr{args.lr}')
        best_path = None
        best_loss = None
